# Blu Keep Servers Plugin (Web Version)

A Vencord plugin that keeps track of servers you've joined and allows you to rejoin them later, even after being kicked or leaving. This is the web version that uses localStorage for data persistence.
...
1. Copy the `blu-keepServers` folder to your Vencord `src/userplugins_` directory
...
Server data is stored in the browser's localStorage for persistence across sessions. The data is saved to:

-   **Storage Key**: `blu-keepServers-data`
-   **Format**: JSON object with server IDs as keys
-   **Persistence**: Survives browser restarts and Discord refreshes
